...
locals {
  ...
  repositoryApp = "${var.appName}-impl"
  repositorySetup = "${var.appName}-setup"
  ...
  apiDomainName = "${var.apiSubDomain[var.env]}.${var.appFrontDomain[var.env]}"
  apiKnownName = "${var.appName}"
}
...
module "runtime" {
  ...
  apiDomainName = "${local.apiDomainName}"
  apiKnownName =  "${local.apiKnownName}"
